...
def are_fibonacci_adjacent(a, b):
    """Check if two numbers are adjacent in the Fibonacci sequence."""
    for i in range(len(FIB_SEQUENCE) - 1):
        if (a == FIB_SEQUENCE[i] and b == FIB_SEQUENCE[i+1]) or \
           (b == FIB_SEQUENCE[i] and a == FIB_SEQUENCE[i+1]):
            return True
    return False
...
class Tile:
    ...
    def get_color(self):
        try:
            index = FIB_SEQUENCE.index(self.value)
        except ValueError:
            index = 0
...
def move_tiles(window, tiles, clock, direction):
    ...
    reverse = False
    delta = (0, 0)
    boundary_check = lambda tile: True
    get_next_tile = lambda tile: None
    merge_check = lambda tile, next_tile: False
    move_check = lambda tile, next_tile: False
    ceil = False
    ...
    while updated:
        ...
        for i, tile in enumerate(sorted_tiles):
            ...
            if not next_tile:
                ...
                are_fibonacci_adjacent(tile.value, next_tile.value)
                ...
                if merge_check(tile, next_tile):
                    ...
                    next_tile.value = tile.value + next_tile.value
                    sorted_tiles.pop(i)
